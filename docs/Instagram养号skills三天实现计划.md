...
目标：把所有非业务 skill 的基础包完成到可被调用。

| 模块 | 实现内容 | 交付物 |
| --- | --- | --- |
| monorepo | 建包、workspace 配置、统一 tsconfig、lint/test 脚本 | 可安装、可编译、可运行测试的 workspace |
| `instagram-core` | 完成接口、类型、错误码、branded id 构造函数 | 通用 `InstagramClient` 接口和公共类型 |
| `sessionRef` | 完成 SQLite schema、resolver、三类 provider session 表 | `SqliteInstagramSessionStore` 和 provider 路由能力 |
| `instagram` | 完成 Cordis `ctx.instagram` 声明、provider 注册接口 | 可注册和读取 `ctx.instagram` |
| `instagram-aiograpi-rest` | 完成 client、请求映射、错误映射、素材规格校验、`./plugin` 入口 | 普通 client 入口和 Cordis 插件入口 |
| `instagram-official-api` | 完成包骨架、接口占位、`NOT_IMPLEMENTED` 返回 | 后续官方 API provider 的占位实现 |
...
| `instagram-content-publish` | `SKILL.md`、文案/图片/视频提示词、内容草稿、发布计划、状态查询计划、定时发布未实现占位 | 内容草稿、发布计划和状态查询计划工具 |
...
| 模块 | 实现内容 | 交付物 |
| --- | --- | --- |
| agent 执行层 | 读取 skill 计划，选择 API provider 或 MCP connector 执行 | 计划到执行的统一入口 |
