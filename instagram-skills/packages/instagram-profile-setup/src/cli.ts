...
import { createProfileSetupToolRegistry } from "./tool-registry.js";
...
interface CliOptions {
  command: CliCommand;
  action?: InstagramProfileSetupToolName;
  inputJson: string;
}
 ...
 * 输入：CLI 参数数组。
 * 输出：进程退出码。
 * 作用：执行 profile setup CLI 并输出结构化 JSON。
 */
 ...
 * 输入：解析后的 CLI 选项。
 * 输出：工具执行结果或工具列表。
 * 作用：把 CLI 命令路由到 profile setup 工具注册表。
 */
async function executeCommand(options: CliOptions): Promise<unknown> {
  ...
  if (options.action === undefined) {
    throw new Error("INSTAGRAM_PROFILE_SETUP_ACTION_REQUIRED");
  }
...
function parseArgs(argv: string[]): CliOptions {
  ...
  if (command !== "list-tools" && command !== "invoke") {
    throw new Error("INSTAGRAM_PROFILE_SETUP_COMMAND_REQUIRED");
  }

  let action: InstagramProfileSetupToolName | undefined;
  let inputJson = "{}";
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (arg === "--action") {
      if (value === undefined) throw new Error("INSTAGRAM_PROFILE_SETUP_ACTION_REQUIRED");
    ...
    if (arg === "--input-json") {
      if (value === undefined) throw new Error("INSTAGRAM_PROFILE_SETUP_INPUT_JSON_REQUIRED");
      inputJson = value;
      index += 1;
      continue;
    }

    throw new Error(`INSTAGRAM_PROFILE_SETUP_UNKNOWN_ARG: ${String(arg)}`);
  }
...
/**
 * 输入：action 字符串。
 * 输出：合法 profile setup 工具名。
 * 作用：拒绝未知工具名。
 */
function readAction(value: string): InstagramProfileSetupToolName {
  if (value === "instagram_profile_generate" || value === "instagram_profile_edit_plan") {
    return value;
  }
  throw new Error(`INSTAGRAM_PROFILE_SETUP_ACTION_UNSUPPORTED: ${value}`);
}
...
function readErrorCode(error: unknown): string {
  ...
  return message.split(":")[0] || "instagram_profile_setup_cli_failed";
}
